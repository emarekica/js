...
**querySelectorAll and limitations of `querySelector`**
...
It is already in the HTML, currently hidden because it has the `hidden` class. If you remove it, it will be visible on the screen.
...
DOM ELEMENTS HAVE PROPERTIES & METHODS ON THEM: [Properties, Methods on DOM element](https://developer.mozilla.org/en-US/docs/Web/API/Element)
  
<br><br>


## KEYPRESS EVENTS (keyboard events)
...
**They are global events**, don't happen on one specific element. For those events, **we usually listen on the whole document**.
